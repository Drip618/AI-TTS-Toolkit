...
一个 **Python 脚本**，帮你一键安装全球最好的 15 个开源 TTS（文字转语音）工具。

**不需要管理员权限，不需要 root，不需要 sudo。** 下载后直接运行。
...
<details>
<summary><strong>不需要管理员权限吗？</strong></summary>

是的！所有工具都安装在用户目录下（~/Documents/AI-TTS-Toolkit/），不需要 sudo、不需要 root。
</details>
